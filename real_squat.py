...
for group in GAINS.values():
    group["kp"] *= SAFETY_GAIN_SCALE
    group["kd"] *= SAFETY_GAIN_SCALE
...
def main():
    ...
    # Fill remaining with hand gains to be safe
    for i in range(len(ACTUATOR_GAINS), 35):
        kps[i] = GAINS["hand"]["kp"]
        kds[i] = GAINS["hand"]["kd"]

    # Initial current targets to prevent sudden snaps
    ...
    print("\nStarting Squat cycle loop. Press Ctrl+C to stop.")
    ...
    try:
        ...
        while True:
            ...
            target_q = pose_schedule(elapsed, q_stand, q_pushup)
